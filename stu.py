import math, sys
from plexapi.myplex import MyPlexAccount
from plexapi.playlist import Playlist
from plexapi.exceptions import NotFound

# Parse Arguments
if len(sys.argv) < 4:
    print("Arguments: PLEX_USER PLEX_PASS PLEX_SERVER [PLEX_PLAYLIST]")
    exit(1)

plex_user = sys.argv[1]
plex_pass = sys.argv[2]
plex_server = sys.argv[3]
plex_playlist = "Complete Star Trek Chronological Playlist"

if len(sys.argv) == 5:
    plex_playlist = sys.argv[4]


# Add a movie from plex to the playlist.
def addmovie(title):
    try:
        item = plex.library.section('Movies').get(title)
    except NotFound:
        errors.append(title)
        print("Failed to add %(title)s" % {'title': title })
    else:
        items.append(item)
        print("Added %(title)s" % {'title': title })

# Add a TV show episode or range of episodes from plex to the playlist.
def addtv(show, season, episode, end = False):
    if not end:
        end = episode
    end = end + 1
    show = "Star Trek: %(show)s" % { 'show': show }
    for number in range(episode, end):
        values = { 'show': show, 'season': season, 'number': number}
        try:
            item = plex.library.section('TV Shows').get(show).episode(season=season, episode=number)
        except NotFound:
            errors.append("%(show)s Season %(season)d Episode %(number)d" % values)
            print("Failed to add %(show)s Season %(season)s Episode %(number)s" % values)
        else:
            items.append(item)
            print("Added %(show)s Season %(season)s Episode %(number)s" % values)

# Login
try:
    account = MyPlexAccount(plex_user, plex_pass)
    plex = account.resource(plex_server).connect()
    print("Logged in to %(server)s as %(user)s" % {'server': plex_server, 'user': plex_user})
except:
    print("Failed to login to %(server)s as %(user)s" % {'server': plex_server, 'user': plex_user})
    exit()

# Delete existing playlist
try:
    plex.playlist(plex_playlist).delete()
    print("Removing existing playlist called '%(playlist)s" % {'playlist': plex_playlist})
except:
    print("No existing playlist called '%(playlist)s" % {'playlist': plex_playlist})

items = []
errors = []

# Order Source: http://thestartrekchronologyproject.blogspot.com/
# Rough Draft, will need to do individual episode ordering later
# TOS is going to be a mess, doing that last
addmovie("null")
addtv("Enterprise", 1, 1, 26)
addtv("Enterprise", 2, 1, 26)
addtv("Enterprise", 3, 1, 24)
addtv("Enterprise", 4, 1, 17)
addtv("Enterprise", 4, 20, 21)
addtv("Short Treks", 2 , 5, 5)
addtv("The Original Series", 0, 1, 1)
addtv("Short Treks", 2 , 2, 2)
addtv("Discovery", 1, 1, 15)
addtv("Short Treks", 1 , 4, 4)
addtv("Discovery", 2, 1, 14)
addtv("Short Treks", 2 , 3, 3)
addtv("The Original Series", 1, 1, 22)
addtv("Short Treks", 2 , 4, 4)
addtv("The Original Series", 1, 23, 29)
addtv("The Original Series", 2, 1, 26)
addtv("The Original Series", 3, 1, 9)
addtv("Enterprise", 4, 18, 19)
#fanedit of the finale removing TNG references (totally optional)
addmovie("Star Trek Enterprise Finale")
addtv("The Original Series", 3, 10, 24)
addtv("The Animated Series", 1, 1, 16)
addtv("The Animated Series", 2, 1, 6)
addmovie("Star Trek: The Motion Picture")
addmovie("Star Trek II: The Wrath of Khan")
addmovie("Star Trek III: The Search for Spock")
addmovie("Star Trek IV: The Voyage Home")
addmovie("Star Trek V: The Final Frontier")
addmovie("Star Trek VI: The Undiscovered Country")
addtv("The Next Generation", 1, 1, 26)
addtv("The Next Generation", 2, 1, 22)
addtv("The Next Generation", 3, 1, 26)
addtv("The Next Generation", 4, 1, 12)
addtv("The Next Generation", 4, 14, 14)
addtv("The Next Generation", 4, 13, 13)
addtv("The Next Generation", 4, 15, 26)
addtv("The Next Generation", 5, 1, 26)
addtv("The Next Generation", 6, 1, 11)
addtv("Deep Space Nine", 1, 1, 5)
addtv("The Next Generation", 6, 12, 12)
addtv("Deep Space Nine", 1, 6, 6)
addtv("The Next Generation", 6, 13, 13)
addtv("Deep Space Nine", 1, 7, 7)
addtv("The Next Generation", 6, 14, 14)
addtv("Deep Space Nine", 1, 8, 8)
addtv("The Next Generation", 6, 15, 15)
addtv("Deep Space Nine", 1, 9, 9)
addtv("The Next Generation", 6, 16, 17)
addtv("Deep Space Nine", 1, 10, 11)
addtv("The Next Generation", 6, 18, 19)
addtv("Deep Space Nine", 1, 12, 14)
addtv("The Next Generation", 6, 20, 22)
addtv("Deep Space Nine", 1, 15, 15)
addtv("The Next Generation", 6, 23, 23)
addtv("Deep Space Nine", 1, 16, 16)
addtv("The Next Generation", 6, 24, 24)
addtv("Deep Space Nine", 1, 18, 18)
addtv("Deep Space Nine", 1, 17, 17)
addtv("Deep Space Nine", 1, 19, 19)
addtv("The Next Generation", 6, 25, 25)
addtv("Deep Space Nine", 1, 20, 20)
addtv("The Next Generation", 6, 26, 26)
addtv("The Next Generation", 7, 1, 1)
addtv("Deep Space Nine", 2, 1, 3)
addtv("The Next Generation", 7, 2, 2)
addtv("The Next Generation", 7, 4, 5)
addtv("Deep Space Nine", 2, 4, 5)
addtv("The Next Generation", 7, 3, 3)
addtv("The Next Generation", 7, 6, 6)
addtv("Deep Space Nine", 2, 6, 6)
addtv("The Next Generation", 7, 7, 7)
addtv("Deep Space Nine", 2, 7, 8)
addtv("The Next Generation", 7, 8, 9)
addtv("Deep Space Nine", 2, 9, 10)
addtv("The Next Generation", 7, 11, 11)
addtv("Deep Space Nine", 2, 11, 12)
addtv("The Next Generation", 7, 10, 10)
addtv("The Next Generation", 7, 13, 13)
addtv("The Next Generation", 7, 12, 12)
addtv("Enterprise", 4, 22, 22)
addtv("Deep Space Nine", 2, 13, 13)
addtv("The Next Generation", 7, 14, 15)
addtv("Deep Space Nine", 2, 15, 15)
addtv("Deep Space Nine", 2, 14, 14)
addtv("Deep Space Nine", 2, 16, 16)
addtv("The Next Generation", 7, 16, 17)
addtv("Deep Space Nine", 2, 17, 17)
addtv("The Next Generation", 7, 18, 18)
addtv("Deep Space Nine", 2, 18, 18)
addtv("The Next Generation", 7, 19, 19)
addtv("Deep Space Nine", 2, 19, 19)
addtv("The Next Generation", 7, 20, 20)
addtv("Deep Space Nine", 2, 20, 21)
addtv("The Next Generation", 7, 21, 22)
addtv("Deep Space Nine", 2, 22, 22)
addtv("The Next Generation", 7, 23, 23)
addtv("Deep Space Nine", 2, 23, 23)
addtv("The Next Generation", 7, 24, 24)
addtv("Deep Space Nine", 2, 24, 25)
addtv("The Next Generation", 7, 25, 26)
addtv("Deep Space Nine", 2, 26, 26)
addtv("Deep Space Nine", 3, 1, 7)
addtv("Voyager", 1, 1, 2)
addtv("Deep Space Nine", 3, 8, 8)
addtv("Voyager", 1, 3, 3)
addtv("Deep Space Nine", 3, 9, 12)
addtv("Voyager", 1, 4, 4)
addtv("Deep Space Nine", 3, 13, 14)
addtv("Voyager", 1, 5, 5)
addtv("Deep Space Nine", 3, 15, 15)
addtv("Voyager", 1, 6, 6)
addtv("Deep Space Nine", 3, 16, 17)
addtv("Voyager", 1, 7, 9)
addmovie("Star Trek: Generations")
addtv("Voyager", 1, 10, 10)
addtv("Deep Space Nine", 3, 18, 18)
addtv("Voyager", 1, 11, 11)
addtv("Deep Space Nine", 3, 19, 19)
addtv("Voyager", 1, 12, 12)
addtv("Deep Space Nine", 3, 20, 21)
addtv("Voyager", 1, 13, 13)
addtv("Deep Space Nine", 3, 22, 22)
addtv("Voyager", 1, 14, 14)
addtv("Deep Space Nine", 3, 23, 23)
addtv("Voyager", 1, 15, 15)
addtv("Deep Space Nine", 3, 24, 24)
addtv("Voyager", 1, 16, 16)
addtv("Deep Space Nine", 3, 25, 25)
addtv("Voyager", 2, 3, 4)
addtv("Deep Space Nine", 3, 26, 26)
addtv("Voyager", 2, 1, 2)
addtv("Voyager", 2, 5, 5)
addtv("Deep Space Nine", 4, 1, 2)
addtv("Voyager", 2, 6, 6)
addtv("Deep Space Nine", 4, 3, 4)
addtv("Voyager", 2, 7, 7)
addtv("Deep Space Nine", 4, 5, 5)
addtv("Voyager", 2, 8, 10)
addtv("Deep Space Nine", 4, 6, 6)
addtv("Voyager", 2, 11, 11)
addtv("Deep Space Nine", 4, 7, 9)
addtv("Voyager", 2, 12, 12)
addtv("Deep Space Nine", 4, 10, 12)
addtv("Voyager", 2, 13, 13)
addtv("Voyager", 2, 18, 18)
addtv("Voyager", 2, 14, 14)
addtv("Deep Space Nine", 4, 13, 13)
addtv("Voyager", 2, 15, 15)
addtv("Deep Space Nine", 4, 14, 14)
addtv("Voyager", 2, 16, 17)
addtv("Voyager", 2, 19, 21)
addtv("Deep Space Nine", 4, 15, 17)
addtv("Voyager", 2, 22, 22)
addtv("Deep Space Nine", 4, 18, 20)
addtv("Voyager", 2, 23, 23)
addtv("Deep Space Nine", 4, 21, 21)
addtv("Voyager", 2, 24, 24)
addtv("Deep Space Nine", 4, 22, 22)
addtv("Voyager", 2, 25, 25)
addtv("Deep Space Nine", 4, 23, 26)
addtv("Voyager", 2, 26, 26)
addtv("Voyager", 3, 1, 1)
addtv("Deep Space Nine", 5, 1, 2)
addtv("Voyager", 3, 7, 7)
addtv("Voyager", 3, 5, 5)
addtv("Voyager", 3, 2, 3)
addtv("Voyager", 3, 6, 6)
addtv("Voyager", 3, 4, 4)
addtv("Deep Space Nine", 5, 4, 4)
addtv("Deep Space Nine", 5, 3, 3)
addtv("Deep Space Nine", 5, 5, 6)
addtv("Voyager", 3, 8, 9)
addtv("Deep Space Nine", 5, 7, 8)
addtv("Voyager", 3, 10, 10)
addtv("Deep Space Nine", 5, 9, 9)
addtv("Voyager", 3, 11, 11)
addtv("Deep Space Nine", 5, 10, 11)
addtv("Voyager", 3, 12, 14)
addtv("Deep Space Nine", 5, 12, 13)
addtv("Voyager", 3, 15, 16)
addmovie("Star Trek: First Contact")
addtv("Deep Space Nine", 5, 14, 15)
addtv("Voyager", 3, 17, 18)
addtv("Deep Space Nine", 5, 16, 16)
addtv("Voyager", 3, 19, 19)
addtv("Deep Space Nine", 5, 17, 19)
addtv("Voyager", 3, 20, 20)
addtv("Deep Space Nine", 5, 20, 22)
addtv("Voyager", 3, 21, 24)
addtv("Deep Space Nine", 5, 23, 23)
addtv("Voyager", 3, 25, 25)
addtv("Deep Space Nine", 5, 24, 24)
addtv("Voyager", 3, 26, 26)
addtv("Voyager", 4, 1, 2)
addtv("Deep Space Nine", 5, 25, 26)
addtv("Voyager", 4, 3, 5)
addtv("Deep Space Nine", 6, 1, 6)
addtv("Voyager", 4, 6, 7)
addtv("Deep Space Nine", 6, 7, 7)
addtv("Voyager", 4, 8, 9)
addtv("Deep Space Nine", 6, 8, 8)
addtv("Voyager", 4, 10, 10)
addtv("Deep Space Nine", 6, 9, 9)
addtv("Voyager", 4, 11, 11)
addtv("Deep Space Nine", 6, 10, 11)
addtv("Voyager", 4, 12, 14)
addtv("Deep Space Nine", 6, 12, 14)
addtv("Voyager", 4, 15, 15)
addtv("Deep Space Nine", 6, 15, 16)
addtv("Voyager", 4, 16, 19)
addtv("Deep Space Nine", 6, 17, 19)
addtv("Voyager", 4, 20, 21)
addtv("Deep Space Nine", 6, 20, 20)
addtv("Voyager", 4, 22, 22)
addtv("Deep Space Nine", 6, 21, 22)
addtv("Voyager", 4, 24, 24)
addtv("Deep Space Nine", 6, 23, 23)
addtv("Voyager", 4, 25, 25)
addtv("Deep Space Nine", 6, 24, 24)
addtv("Voyager", 4, 26, 26)
addtv("Deep Space Nine", 6, 25, 26)
addtv("Voyager", 5, 1, 5)
addtv("Voyager", 5, 8, 8)
addtv("Voyager", 5, 6, 6)
addtv("Deep Space Nine", 7, 1, 8)
addtv("Voyager", 5, 9, 9)
addtv("Deep Space Nine", 7, 9, 9)
addtv("Voyager", 5, 7, 7)
addtv("Voyager", 5, 10, 10)
addtv("Deep Space Nine", 7, 10, 10)
addmovie("Star Trek: Insurrection")
addtv("Deep Space Nine", 7, 11, 11)
addtv("Voyager", 5, 11, 12)
addtv("Deep Space Nine", 7, 12, 12)
addtv("Voyager", 5, 13, 13)
addtv("Deep Space Nine", 7, 13, 13)
addtv("Voyager", 5, 14, 14)
addtv("Deep Space Nine", 7, 15, 16)
addtv("Voyager", 5, 17, 17)
addtv("Deep Space Nine", 7, 15, 16)
addtv("Voyager", 5, 18, 20)
addtv("Deep Space Nine", 7, 17, 26)
addtv("Voyager", 5, 21, 26)
addtv("Voyager", 6, 1, 26)
addtv("Voyager", 7, 1, 26)
addmovie("Star Trek: Nemesis")
addtv("Lower Decks", 1, 1, 10)
addmovie("Star Trek")
addmovie("Star Trek Into Darkness")
addmovie("Star Trek Beyond")
addtv("Short Treks", 2, 6, 6)
addtv("Picard", 1, 1, 10)
addtv("Voyager", 4, 23, 23)
addtv("Discovery", 3, 1, 13)

print("----------------------------------------------------")

# Create playlist
if len(items) > 0:
    playlist = Playlist.create(plex, plex_playlist, items)
else:
    print("Script couldn't find any items to add to your Star Trek playlist")
    exit()

# Get playlist duration
hours = math.floor(playlist.duration / 1000 / 60 / 60)
time = "%(days)d days and %(hours)d hours" % { 'days': math.floor(hours / 24), 'hours': hours % 24 }

if len(errors) > 0:
    print("The following files could not be found to add to the playlist.")
    for item in errors:
        print("- %(item)s" % { 'item': item })
    print("Run the script again once these items are in Plex.")

print("----------------------------------------------------")
print("Enjoy %(time)s of Star Trek!" % {'time': time})
